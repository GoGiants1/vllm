    ...
    def _load_model_serialized(
        ...
        with set_default_torch_dtype(model_config.dtype):
            with torch.device(device_config.device):
                ...
                model = load_with_tensorizer(
                    tensorizer_config, vllm_config=vllm_config
                )
        return model.eval()
...
class BitsAndBytesModelLoader(BaseModelLoader):
    ...
    def _get_quantized_weights_iterator(
        ...
        try:
            ...
            raise ImportError(
                "Please install bitsandbytes>=0.45.0 via "
    ...
    def _is_8bit_weight_name(self, weight_name: str):
        ...
        return any(
            weight_name.lower().endswith(suffix) for suffix in quantized_suffix
        )
    ...
    def _quantized_4bit_generator(
        ...
        weight_iterator = self._hf_weight_iter(
        ...
        def _parse_quant_state(
            param_name: str, temp_state_dict: Dict
        ) -> QuantState:
            quant_state = {}
        ...
        for (
            ...
            if (
                f"{mapped_weight_name}.quant_state.bitsandbytes__nf4"
                in temp_state_dict
            ) or (
                f"{mapped_weight_name}.quant_state.bitsandbytes__fp4"
                in temp_state_dict
            ):
                quant_state = _parse_quant_state(
                    mapped_weight_name, temp_state_dict
                )
            ...
            if any(
                ...
                # weight and module use same name.
                    ...
                    shard_weights_index = [
                        ...
                        for idx, size in zip(
                            total_start_index, total_shard_sizes
                        )
                    ]
    ...
    def _load_weights(
        ...
        for name, module in model.named_modules():
            ...
            # modules to shard the weights correctly.
                ...
                module, (QKVParallelLinear, MergedColumnParallelLinear)
            ):
        ...
        for param_name, param in param_dict.items():
            if param_name in stacked_quant_state_dict:
                ...
                for seq, quant_state in quant_states.items():
                    num_elements[seq] = (
                        math.prod(quant_state.shape) // pack_ratio
                    )
...
class RunaiModelStreamerLoader(BaseModelLoader):
    ...
    def __init__(self, load_config: LoadConfig):
        ...
        if load_config.model_loader_extra_config:
            ...
            if (
                runai_streamer_s3_endpoint is None
    ...
    def _prepare_weights(
        ...
        if is_s3_path:
            hf_weights_files = s3_glob(
                path=hf_folder, allow_pattern=[safetensors_pattern]
            )
        else:
